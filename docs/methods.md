...
The authors' MATLAB works in **metres**, so every threshold is checkable. Defaults are
...
- Smoothed with a Gaussian, **σ = 7.5 cm** (re-scaled from the paper's 2.5 cm).
- Optional **10-fold cross-validation** (paper's method) picks the σ that maximises held-out decoding (`--cv-smoothing`,
  off by default).
...
- **Phase (default, ours):** LFP band-passed **5–10 Hz** (2nd-order Butterworth) → Hilbert transform
  → instantaneous phase.
...
Two input matrices:
- **Spike counts** $\mathbf{n}$: $N$ units × $T$ bins, Gaussian-smoothed over **~50 ms** (5 bins).
  *The paper's PV decoder uses a lighter ~10 ms (1-bin) smoothing; the heavier 50 ms follows Tang et
  al.'s Bayesian variant.*
...
- A bin is kept only if its peak clears the **99th-percentile spike-shuffle null**, has **≥ 1 active
  cell** (paper's approach), and its low-pass anchor lies **< 50 cm** from the true position.
...
- **Straightness of the sweep candidate** — the fraction of the decoded positions' variance that lies
  along the sweep axis; accepted if $r^2 > 0.5$:

$$r^2 \;=\; 1 - \frac{\mathrm{Var}(e)}{\mathrm{Var}(x) + \mathrm{Var}(y)}$$

  where $e$ is the perpendicular residual to the fitted sweep axis and $x, y$ are the decoded
  coordinates.
